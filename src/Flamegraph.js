...
class FlameRect extends React.Component {
  ...
  hideLabel() {
    if (
      ...
      this.labelRef.current.getBoundingClientRect().width > this.props.width
    ) {
  ...
  render() {
    ...
    return (
      ...
      >
        ...
        />
        {!hideLabel && (
          <text x={5} y={13} style={{ fontSize: "12px" }} ref={this.labelRef}>
            {name}
          </text>
        )}
      </g>
...
class Flamegraph extends React.Component {
  ...
  render() {
    ...
    return (
      ...
      >
        {data.map((d, i) => {
          const start = data.slice(0, i).reduce((sum, d) => sum + d.value, 0);
